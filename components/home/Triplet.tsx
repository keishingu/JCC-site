...
export default function HomeTriplet({ journals, news }: { journals: JournalIssue[]; news: NewsItem[] }) {
  return (
    <section style={{
      padding: '70px 56px 50px', background: 'var(--color-paper)',
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 22 }}>
          <h2 style={{ margin: 0, fontSize: 26, letterSpacing: '.06em', fontWeight: 500, color: 'var(--color-ink)',
            fontFamily: 'var(--font-en-sans)' }}>JOURNAL</h2>
          <span style={{ fontSize: 13, color: 'var(--color-slate)' }}>会誌・会報</span>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
          {journals.slice(0, 4).map((j) => (
            <div key={j.id}>
              <JournalCover issue={j}/>
              <div style={{ marginTop: 10, fontSize: 12, fontWeight: 500, color: 'var(--color-ink)',
                fontFamily: 'var(--font-en-serif)' }}>No.{j.issueNumber}</div>
              <div style={{ fontSize: 10.5, color: 'var(--color-slate)' }}>{j.season}</div>
            </div>
          ))}
        </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 22 }}>
          <h2 style={{ margin: 0, fontSize: 26, letterSpacing: '.06em', fontWeight: 500, color: 'var(--color-ink)',
            fontFamily: 'var(--font-en-sans)' }}>NEWS</h2>
          <span style={{ fontSize: 13, color: 'var(--color-slate)' }}>お知らせ・Field Notes</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {news.slice(0, 5).map((n, i) => (
            <Link key={n.id} href={`/news/${n.slug}`} style={{
              ...
              <span style={{ fontSize: 10.5, letterSpacing: '.04em', color: 'var(--color-slate)',
                fontFamily: 'var(--font-en-serif)' }}>{formatDate(n.publishedAt)}</span>
              <span style={{
                fontSize: 9, letterSpacing: '.12em', color: 'var(--color-navy)',
                border: '1px solid rgba(10,22,40,0.08)', padding: '3px 6px',
                textAlign: 'center', justifySelf: 'start',
                fontFamily: 'var(--font-en-serif)',
              }}>{tagLabels[n.tag] ?? n.tag}</span>
              <span style={{ fontSize: 12.5, lineHeight: 1.45, color: 'var(--color-ink)',
                fontFamily: 'var(--font-jp)' }}>{n.title}</span>
            </Link>
          ))}
        </div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 16, marginBottom: 22 }}>
          <h2 style={{ margin: 0, fontSize: 26, letterSpacing: '.06em', fontWeight: 500, color: 'var(--color-ink)',
            fontFamily: 'var(--font-en-sans)' }}>CLIMBERS</h2>
          <span style={{ fontSize: 13, color: 'var(--color-slate)' }}>会員紹介</span>
        </div>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={{ fontSize: 14, color: 'var(--color-ink)', fontFamily: 'var(--font-jp)' }}>会員紹介</div>
          <div style={{ fontSize: 11.5, color: 'var(--color-slate)', lineHeight: 1.7, marginTop: 4 }}>JCCの活動を支える会員たちの紹介です。</div>
        </div>
